            ...
            if annotations:
                ...
                for annotation in annotations:
                    entity.contextual_annotations.append(
                        {"category": annotation.category, "annotations": annotation.annotations}
                    )
...
class CodeKnowledgeGraph:
        ...
        for entity in entities:
            ...
            """,
                (
                    ...
                    entity.team_ownership,
                    entity.importance_score,
                    (
                        json.dumps(entity.contextual_annotations)
                        if entity.contextual_annotations
                        else None
                    ),
                ),
            )
    ...
    def graph_search(
        ...
        def traverse(current_id, current_depth):
            ...
            if entity:
                result["nodes"].append(
                    {"id": entity[0], "type": entity[1], "name": entity[2], "complexity": entity[6]}
                )
    ...
    def contextual_search(
        ...
        if context_filters:
            ...
            if (
                "has_security_annotations" in context_filters
                and context_filters["has_security_annotations"]
            ):
    ...
    def get_team_analytics(self) -> Dict[str, Any]:
        ...
        for row in cursor:
            ...
            teams[team] = {
                ...
                "quality_score": (
                    round((count - quality_issues - style_issues) / count * 100, 1)
                    if count > 0
                    else 100
                ),
            }
...
class NaturalLanguageQueryProcessor(AIQueryProcessor):
    ...
    def _find_security_critical_code(self, kg: "CodeKnowledgeGraph") -> List[Dict]:
        ...
        return kg.contextual_search(
            "security authentication", {"has_security_annotations": True, "min_importance": 1.5}
        )
    ...
    def _calculate_confidence(self, entities: List[Dict], intent: str, query: str) -> float:
        ...
        if entities and isinstance(entities[0], dict):
            avg_importance = sum(e.get("importance_score", 1.0) for e in entities[:5]) / min(
                5, len(entities)
            )
    ...
    def _generate_code_overview(self, kg: "CodeKnowledgeGraph") -> List[Dict]:
        ...
        overview = {
            ...
            "summary": self._generate_codebase_description(
                stats, key_entities, domains, main_files
            ),
        }
    ...
    def _generate_codebase_description(self, stats, key_entities, domains, main_files) -> str:
        ...
        description_parts.append(
            f"This codebase contains {total_entities} code entities across {file_count} Python files."
        )

        if class_count > 0:
            description_parts.append(
                f"It includes {class_count} classes with {method_count} methods, plus {function_count} standalone functions."
            )
        else:
            description_parts.append(
                f"It consists primarily of {function_count} functions organized in a functional programming style."
            )
            ...
            for entity in key_entities[:5]:
                if entity.get("docstring"):
                    ...
                    key_components.append(
                        f"• {entity['name']} ({entity['type']}): {first_sentence}"
                    )
                else:
        ...
        if domains:
            ...
            description_parts.append(
                f"The code is organized around these business domains: {', '.join(domain_list)}."
            )
        ...
        if main_files:
            ...
            if any(
                name.lower() in ["main.py", "__main__.py", "app.py", "server.py"]
                for name in main_file_names
            ):
                description_parts.append(
                    "This appears to be an application with a main entry point."
                )
            if any(
                name.lower() in ["pipeline.py", "processor.py", "analyzer.py"]
                for name in main_file_names
            ):
                description_parts.append(
                    "The codebase implements data processing or analysis pipelines."
                )
    ...
    def _format_natural(self, result: QueryResult) -> str:
        ...
        if result.intent == QueryIntent.CODE_OVERVIEW:
            if result.entities and result.entities[0].get("type") == "overview":
                ...
                if overview.get("summary"):
                    ...
                    for line in summary_lines:
                        if line.strip():
                            response.append(
                                f"   {line.strip()}{'.' if not line.endswith('.') else ''}"
                            )
                    response.append("")
            ...
            for pattern in result.entities:
                response.append(
                    f"   • {pattern['pattern']}: {pattern['entity_count']} entities (avg complexity: {pattern['avg_complexity']})"
                )
            ...
            for entity in result.entities[:10]:
                domain_info = (
                    f" ({entity.get('business_domain', 'Unknown')})"
                    if entity.get("business_domain")
                    else ""
                )
                team_info = (
                    f" [Team: {entity.get('team_ownership')}]"
                    if entity.get("team_ownership")
                    else ""
                )
                response.append(
                    f"   • {entity.get('name', 'Unknown')} ({entity.get('type', 'unknown')}){domain_info}{team_info}"
                )
            ...
            for entity in result.entities[:10]:
                response.append(
                    f"   • {entity.get('name', 'Unknown')} (complexity: {entity.get('complexity', 0)})"
                )
            ...
            for entity in result.entities[:10]:
                ...
                response.append(
                    f"   • {entity.get('name', 'Unknown')} ({entity.get('type', 'unknown')}) - Score: {importance:.2f}"
                )
                if entity.get("business_domain"):
    ...
    def _format_structured(self, result: QueryResult) -> str:
        ...
        if result.entities:
            ...
            for i, entity in enumerate(result.entities[:15], 1):
                lines.append(
                    f"{i:2d}. {entity.get('name', 'Unknown')} ({entity.get('type', 'unknown')})"
                )
