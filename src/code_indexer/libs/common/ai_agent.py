...
class NaturalLanguageQueryProcessor(AIQueryProcessor):
    ...
    def process(self, query: str, knowledge_graph: CodeKnowledgeGraph) -> QueryResult:
        ...
        if intent == QueryIntent.FIND_FUNCTION:
            entities = self._find_entities_by_name(
                knowledge_graph, extracted_terms, entity_type="function"
            )
        elif intent == QueryIntent.FIND_CLASS:
            entities = self._find_entities_by_name(
                knowledge_graph, extracted_terms, entity_type="class"
            )
    ...
    def _find_entities_by_name(
        self, kg: CodeKnowledgeGraph, terms: List[str], entity_type: str = None
    ) -> List[Dict]:
    ...
    def _find_security_critical_code(self, kg: CodeKnowledgeGraph) -> List[Dict]:
        ...
        return kg.contextual_search("security authentication", {"min_importance": 1.5})
    ...
    def _find_by_architectural_pattern(
        self, kg: CodeKnowledgeGraph, terms: List[str]
    ) -> List[Dict]:
    ...
    def _generate_code_overview(self, kg: CodeKnowledgeGraph) -> List[Dict]:
        ...
        overview = {"statistics": stats, "key_entities": key_entities}
...
class CodebaseAIAgent:
    ...
    def __init__(self, knowledge_graph: CodeKnowledgeGraph):
        ...
        self.processors = [NaturalLanguageQueryProcessor()]
