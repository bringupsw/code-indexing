...
class CodeKnowledgeGraph:
    ...
    def store_entities_and_embeddings(self, entities: List[CodeEntity], 
        ...
        for entity in entities:
            ...
            ''', (
                ...
                entity.line_start, entity.line_end, entity.source_code, entity.docstring,
                entity.signature, entity.complexity, 
                json.dumps(entity.parameters), entity.return_type,
                json.dumps(entity.decorators), entity.class_name,
                json.dumps(entity.imports), json.dumps(entity.calls),
                json.dumps(entity.variables), entity.has_docstring,
                entity.is_public, entity.is_tested, entity.handles_exceptions,
                entity.has_type_hints, entity.business_domain, entity.team_owner,
                entity.architectural_pattern, entity.importance_score,
                json.dumps(entity.quality_issues), json.dumps(entity.annotations),
                embedding_blob
            ))
    ...
    def store_relations(self, relations: List[CodeRelation]):
        ...
        for relation in relations:
            ...
            ''', (
                ...
                relation.weight, json.dumps(relation.metadata)
            ))
    ...
    def contextual_search(self, query: str, context_filters: Dict[str, Any] = None, 
        ...
        results = []
        for row in cursor:
            (entity_id, name, entity_type, business_domain, team_owner,
             importance_score, annotations, embedding_blob,
    ...
    def get_entity_by_id(self, entity_id: str) -> Optional[Dict]:
        ...
        if row:
            ...
            # Parse JSON fields
            for field in ['parameters', 'decorators', 'imports', 'calls', 'variables', 'quality_issues', 'annotations']:
    ...
    def get_statistics(self) -> Dict:
        ...
        cursor = conn.execute('SELECT relation_type, COUNT(*) FROM relations GROUP BY relation_type')
        stats['relation_types'] = dict(cursor.fetchall())
        
        # Business domain statistics
        cursor = conn.execute('SELECT business_domain, COUNT(*) FROM entities WHERE business_domain IS NOT NULL GROUP BY business_domain')
        stats['business_domains'] = dict(cursor.fetchall())
        
        # Complexity statistics
        cursor = conn.execute('SELECT AVG(complexity), MIN(complexity), MAX(complexity) FROM entities')
